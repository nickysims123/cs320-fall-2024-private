...
let type_of expr =
  let rec typecheck env expr =
      ...
      | Let { is_rec; name; ty = expected_ty; value; body } -> (
          ...
          in
          match typecheck added_env value with
          | Ok actual_ty ->
              if actual_ty = expected_ty then
                  typecheck (Env.add name expected_ty added_env) body
              else Error (LetTyErr (expected_ty, actual_ty))
          | Error e -> Error e
      )
      | Fun (arg, arg_ty, body) ->
          let added_env = Env.add arg arg_ty env in
          ( match typecheck added_env body with
...
let eval expr =
  let rec eval_expr env expr =
    ...
    | Let { is_rec; name; ty = _; value; body } ->
      ...
      in
      let v = eval_expr closed_env value in
      let added_env = Env.add name v closed_env in
